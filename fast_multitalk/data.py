...
def save_video_ffmpeg(
    ...
    audio_save_path = os.path.join(os.path.dirname(save_path), f"{uuid.uuid4()}.wav")
    ...
    duration = T / fps
    save_path_crop_audio = os.path.join(
        os.path.dirname(save_path), f"{uuid.uuid4()}.wav"
    )
    final_command = [
        "ffmpeg",
        "-i",
    ...
    subprocess.run(
        final_command, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
